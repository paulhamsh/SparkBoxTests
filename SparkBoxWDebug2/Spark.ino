    ...
    switch (cmdsub) {
      case 0x0201:  
         ...
         else if (ui_update_in_progress == UI_HARDWARE)
         {
           ...
           app_msg_out.create_preset(&presets[pres]);
           app_process();
           delay(100);
           
           app_msg_out.change_hardware_preset(0x00, pres);
           app_process();
...
void update_ui_hardware(int pres)
{
  if (pres <= 3) {
    sp_bin.pass_through = false;
    app_bin.pass_through = false;
    
    app_msg_out.save_hardware_preset(0x00, pres);
    app_process();
